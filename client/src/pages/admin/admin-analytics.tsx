...
export default function AdminAnalyticsPage() {
  ...
  const { data: peakTimesData, isLoading: isLoadingPeakTimes } = useQuery({
  ...
  const calculateTotalRevenue = () => {
    ...
    return revenueData.reduce(
      (total: number, item: RevenueData) => total + item.amount,
      0,
    );
  };
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoadingPeakTimes ? (
                  ...
                  formatHour(
                    peakTimesData.sort(
                      (a: TimeData, b: TimeData) => b.count - a.count,
                    )[0].hour,
                  )
                ) : (
                  "N/A"
          ...
          <TabsContent value="locations">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Top locations by order volume</CardDescription>
              </CardHeader>
                    ...
                    <div className="w-full lg:w-1/2 h-80">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          >
                            {locationsData.map(
                              (entry: LocationData, index: number) => (
                                <Cell
                                  key={`cell-${index}`}
                          ...
                          (location: LocationData, index: number) => (
                            <li key={location.id} className="flex items-center">
                              ...
                              <div className="text-right font-medium">
                                ...
                                {formatPercent(location.percentage)})
                              </div>
                            </li>
                          ),
                        )}
