...
type InvalidateQueryFn<TVariables> = (
  variables: TVariables,
) => string | string[] | readonly unknown[][];

interface UseMutationHookOptions<TData, TVariables>
  extends Omit<
    ...
    if (invalidateQueries) {
      if (typeof invalidateQueries === "function") {
        ...
        if (Array.isArray(queryKeys)) {
          ...
          if (isNestedArray) {
            await Promise.all(
              (queryKeys as readonly unknown[][]).map((key) =>
                queryClient.invalidateQueries({ queryKey: key }),
              ),
            );
          } else {
            await Promise.all(
              (queryKeys as string[]).map((key) =>
                queryClient.invalidateQueries({ queryKey: [key] }),
              ),
            );
          }
