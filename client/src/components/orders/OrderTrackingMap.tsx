  ...
  useEffect(() => {
    ...
    if (user && user.id) {
      ...
      orderTrackingService.on("connected", () => {
        ...
        console.log(
          "Connected to tracking service, starting to track order:",
          orderId,
        );
      ...
      <CardContent className="p-4">
        ...
        <div className="relative overflow-hidden rounded-lg bg-neutral-100 w-full h-48">
          {isConnecting ? (
            ...
            <div
              ref={mapRef}
              className="w-full h-full bg-neutral-100 flex items-center justify-center relative"
            >
              <div className="text-xs text-neutral-500 text-center absolute top-2 left-2 right-2">
                Map showing driver at position {driverLocation.lat.toFixed(6)},{" "}
