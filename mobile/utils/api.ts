...
export const auth = {
  login: async (username: string, password: string) => {
    return apiRequest<{ id: number; username: string }>('/login', 'POST', { username, password });
  },
  ...
  getCurrentUser: async () => {
    return apiRequest<{ id: number; username: string } | null>('/user')
...
export const fuel = {
  getPrices: async (state: string = 'FL') => {
    return apiRequest<{ REGULAR_UNLEADED: number; PREMIUM_UNLEADED: number; DIESEL: number }>(`/fuel-prices?state=${state}`);
  }
};
